...
past_layouts.append(cur_layout)
move = 0
# for i in range(4):
while True:
    move += 1
    ...
    for lay in search_tree[-2]:
        ...
        for i in range(len(equipment)):
            if floor > 0:
                ...
                if check_valid(new_layout) and new_layout not in past_layouts:
                    search_tree[-1].append(deepcopy(new_layout))
                    past_layouts.append(deepcopy(new_layout))
                    if not solution:
            ...
            if floor + 1 < len(cur_layout):
                ...
                if check_valid(new_layout_2) and new_layout_2 not in past_layouts:
                    search_tree[-1].append(deepcopy(new_layout_2))
                    past_layouts.append(deepcopy(new_layout_2))
                    if not solution:
        ...
        for i in range(len(two_eq)):
            if floor > 0:
                ...
                if check_valid(new_layout_3) and new_layout_3 not in past_layouts:
                    search_tree[-1].append(deepcopy(new_layout_3))
                    past_layouts.append(deepcopy(new_layout_3))
                    if not solution:
                        solution = check_solution(new_layout_3)
            if floor + 1 < len(cur_layout):
                new_layout_4 = deepcopy(cur_layout)
                up_two(new_layout_4, floor, two_eq[i])
                if check_valid(new_layout_4) and new_layout_4 not in past_layouts:
                    search_tree[-1].append(deepcopy(new_layout_4))
                    past_layouts.append(deepcopy(new_layout_4))
                    if not solution:
